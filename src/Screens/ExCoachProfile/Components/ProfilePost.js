...
function ProfilePost(props) {
  ...
  const generateLink = async (item) => {
    ...
    var companyName = item?.userData[0].companyName.split(' ').slice(0, -1).join(' ');
    let cName = companyName === "" ? item?.userData[0].companyName:companyName
        ...
        {isImage(item) ? (
          ...
          <View style={styles.content}>
            ...
            <TouchableWithoutFeedback onPress={() => navigation.navigate("PostDetailScreen", {
             ...
             <TouchableOpacity style={{position: 'absolute', right: 10, bottom:240 }} onPress={() => setSound(!sound)}>
                 <Icon
    ...
    return (
      <View>
        <FlatList
          ...
          renderItem={({ item, index }) => ItemView(item, index, indexData, itemData)}
          horizontal={true}
          pagingEnabled={true}
          bounces={false}
          onScroll={onScroll}
...
<View style={{ flexWrap:'wrap-reverse', marginLeft:20, left:10}}>
...
<View style={{ borderRadius:50, width:40, height:40,left:20}} />
<View style={{ borderRadius:50, width:40, height:40,left:60}} />
</View>

<View style={{ flexWrap:'wrap-reverse', marginRight:20, bottom:50 }}>
